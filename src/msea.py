...
class Domain:
    ...
    def add_points(self, points, projection):
        ...
        if not projection.IsSame(self._projection):
            ...
            points = osr.CoordinateTransformation(
                projection, self._projection).TransformPoints(points)
    ...
    def unrefine_boundaries(self, x0, y0, sampling, mesh_size_cb):
        ...
        tag = np.hstack(list(
            np.full(c.points.shape[0], i, np.int32) for i, c in enumerate(self._curves)))
        ...
        xo = np.ctypeslib.frombuffer(c.cast(p_xo, c.POINTER(
            n_xo.value*2*c.c_double)).contents, dtype=np.float64).reshape([-1, 2]).copy()
        lib.libcfree(p_xo)
        l = np.ctypeslib.frombuffer(c.cast(p_l, c.POINTER(
            n_l.value*3*c.c_int)).contents, dtype=np.int32).reshape([-1, 3]).copy()
        lib.libcfree(p_l)
...
def mesh_gmsh(domain, filename, mesh_size, version=4.0, gmsh_options={}):
    ...
    for i in range(nadapt):
        ...
        data = np.column_stack(
            [node_x[tri, :].swapaxes(1, 2).reshape([-1, 9]), node_lc[tri]])
